...
// export function token_to_address(token: Token): usize {  
export function token_to_address(token: Token): Address {
    assert(token.kind == TokenKind.ADDRESS, "Token is not an address.");
    // let a: Address = changetype<Address>(token.data as u32);

    return changetype<Address>(token.data as u32);
    // return token.data as u32
    // return a;
    // return changetype<Address>(token.data as u32 || 816);
    // return load<Address>(changetype<usize>(token.data as u32));
    // return changetype<Address>(token.data as u32 || 816);
}

// export function token_to_address2(token: Token): Address {  
//     assert(token.kind == TokenKind.ADDRESS, "Token is not an address.");
//     // return changetype<Address>(token.data as u32);
//     return changetype<Address>(816);
// }
...
// export function token_from_address(address: Address): u32 {  
    // let token: Token;
    let token = new Token();
    token.kind = TokenKind.ADDRESS;
    token.data = changetype<u32>(address);
    return token
    // return changetype<u32>(token)
    // return 816
}
