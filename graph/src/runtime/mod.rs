...
pub trait AscType: Sized {
    ...
    where
        Self: AscIndexId,
    {
        // println!("Self::INDEX_ASC_TYPE_ID: {:?}", Self::INDEX_ASC_TYPE_ID);
        // println!("ptr.wasm_ptr: {}", ptr.wasm_ptr());
        // println!(
        //     "size_of, cause why not: {}",
        ...
        // let header = heap.get(ptr.wasm_ptr() - 4, 4)?;
        // let rt_size = header.get(0..4).unwrap(); // safe unwrap because line above already errors on out of bounds access
